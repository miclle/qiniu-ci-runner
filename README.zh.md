...
7. 在**管理控制台**中确认 5 个内置 Qiniu managed Runner Specs。4 个标准公共模板已通过双区域 release gate。4 个 `-large` 变体是已文档化、通过自定义 spec 路径配置的 operator Runner Specs；其 80 GiB 物理模板通过区域发布门槛后，再启用供普通 workflow 使用。operator 可调整启用状态、并发与 idle capacity。
...
| 模板                                   | 说明                                          |
| -------------------------------------- | --------------------------------------------- |
| `templates/github-runner-ubuntu-slim`  | 创建时请求 20 GiB 磁盘的 Ubuntu Slim x64 Runner 模板 |
| `templates/github-runner-ubuntu-22.04` | 创建时请求 20 GiB 磁盘的 Ubuntu 22.04 x64 Runner 模板 |
| `templates/github-runner-ubuntu-24.04` | 创建时请求 20 GiB 磁盘的 Ubuntu 24.04 x64 Runner 模板 |
| `templates/github-runner-ubuntu-26.04` | 创建时请求 20 GiB 磁盘的预览版 Ubuntu 26.04 x64 Runner 模板 |
| `templates/github-runner-ubuntu-slim-large` | 创建时请求 80 GiB 磁盘的 Ubuntu Slim x64 Runner 模板 |
| `templates/github-runner-ubuntu-22.04-large` | 创建时请求 80 GiB 磁盘的 Ubuntu 22.04 x64 Runner 模板 |
| `templates/github-runner-ubuntu-24.04-large` | 创建时请求 80 GiB 磁盘的 Ubuntu 24.04 x64 Runner 模板 |
| `templates/github-runner-ubuntu-26.04-large` | 创建时请求 80 GiB 磁盘的 Ubuntu 26.04 x64 Runner 模板 |
...
`disk_size_mb = 20480`，large 配置请求 `81920`。qshell 在 rebuild 时忽略磁盘
大小，因此实际磁盘与配置不符的同名模板须另行迁移物理模板，才能发布本次配置。
