...
- Ordinary-user routes 暴露 `/` 的 PR/job dashboard、`/github/pulls/{owner}/{repo}/{number}/jobs` 这类 stable GitHub-context job-group routes、`/repositories` 的 local activity repositories、`/account/repositories` 的 GitHub App account setup，以及 `/account/preferences` 和 `/organizations/{login}/preferences` 的 account 或 organization Sandbox service Preferences。
