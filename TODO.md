...
- Complete [Issue #93](https://github.com/qiniu/ci-runner/issues/93) release promotion after the shared-source Actions Runner 2.337.0 change. The Ubuntu 24.04 development template version `20260917.3` reached `Status: ready` on 2026-09-17 (template `w54qc828mejc6x8sgacn`, build `56114e2c-b41d-4239-aa6b-134a8da39c31`), and all nine Sandbox smoke checks passed against the configured endpoint. Remaining: migrate existing same-name standard and `-large` physical templates whose disks differ from the new 20,480-MiB and 81,920-MiB requests, retaining referenced IDs; run release smoke in `cn-yangzhou-1` and `us-south-1`; verify representative workflows and cleanup; then publish and check the regional catalogs. The single dev canary does not establish the full release gate.
